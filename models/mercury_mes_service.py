...
class MercuryMessService(models.AbstractModel):
    ...
    def _map_odoo_country_to_mes(self, country_record):
        ...
        # Fallback to Odoo XML ID if available (more robust)
        # Requires knowing the XML IDs used by Odoo for countries (e.g., base.za)
        # This is more complex but more reliable if XML IDs are known.
        # Example (conceptual):
        # xml_id = self.env['ir.model.data']._xmlid_lookup(country_record)
        # if xml_id:
        #     xml_id_name = xml_id[1] # Get the actual XML ID string
        #     # Map XML ID to MES ID
        #     xml_mapping = {
        #         'base.zm': 3,
        #         'base.za': 142,
        #         # ...
        #     }
        #     return xml_mapping.get(xml_id_name)
